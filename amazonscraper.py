...
# This function 'scrape_amazon' is designed to scrape content from a given Amazon URL using HTTP headers to mimic a browser request.
# It includes error handling for access issues and returns the extracted data using the 'e_amazon.extract' method.
...
with open('laptop_urls.txt', 'w') as text_file:
    data = scrape_search_results('https://www.amazon.in/s?k=laptops')  #url for amazon website which search laptops
    if data:
...
with open("laptop_urls.txt", 'r') as urllist:
